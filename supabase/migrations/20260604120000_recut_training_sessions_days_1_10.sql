...
--   Track days to the LIGHTER format KC locked after beta feedback that the
--   daily content was "too much reading." Days 1-10 previously ran ~280-320
--   words each (5-7 phone screens of scroll); they now run 152-178 words
--   (~1.5 scrolls) inside a hard 190-word ceiling.
--
-- What changed / what did NOT:
--   - CHANGED: mental_skill_md only. Tighter scripture exposition, one example
--     instead of stacked examples, the single-move reset set off on its own
--     line, one landing line. All 5 structural elements survive — the verse
--     stays IN the card (no progressive-disclosure fold; that render concern
--     is deferred to the daily-training UI build).
...
--
-- Authoring:
--   Re-cut under content-curator orchestration to the FV-23-locked format
--   (target 140-170w, hard ceiling 190w). Reviewed by KC (Tier-2, athlete-
--   facing voice). The new format is now the ceiling for FV-9 (days 11-30).
--
-- Idempotency:
...
update public.training_sessions_catalog
   set mental_skill_md = $session$A bad day should tell you what happened. It should not tell you who you are.
...
-- ---------------------------------------------------------------------------
update public.training_sessions_catalog
   set mental_skill_md = $session$Identity is received, not achieved.
...
-- ---------------------------------------------------------------------------
update public.training_sessions_catalog
   set mental_skill_md = $session$A great night is a gift to carry. It is not a crown to put on your own head.
...
-- ---------------------------------------------------------------------------
update public.training_sessions_catalog
   set mental_skill_md = $session$Most of your training happens with no one watching. That isn't lesser work. That's the work that actually forms you.
...
-- ---------------------------------------------------------------------------
update public.training_sessions_catalog
   set mental_skill_md = $session$Comparison isn't measuring you. It's auditioning a verdict on who you are, using someone else's tape.

It runs every direction at once — up at the player ahead of you, sideways at your linemate, down at the one chasing your spot. Your brain will keep generating the question. You decide whether to answer it.
...
You've been named, restored, and called in Christ. The player above you can't add to that. The one chasing you can't subtract from it.
...
-- ---------------------------------------------------------------------------
update public.training_sessions_catalog
   set mental_skill_md = $session$The God who meets you under the lights also meets you in the empty weight room. Same gaze. Same delight.
...
-- ---------------------------------------------------------------------------
update public.training_sessions_catalog
   set mental_skill_md = $session$Rest is not something you earn. It's something you receive.
...
-- ---------------------------------------------------------------------------
-- Day 8 — "Fully Known, Fully Held" (Psalm 139:1-3)
-- ---------------------------------------------------------------------------
update public.training_sessions_catalog
   set mental_skill_md = $session$You carry two versions of yourself most days. The polished one you show coaches and post online. The harsh one that runs you down at 11pm. Neither is the real you.
...
-- ---------------------------------------------------------------------------
-- Day 9 — "Grace Meets the Mistake" (1 John 1:9) — dual register preserved
-- ---------------------------------------------------------------------------
update public.training_sessions_catalog
   set mental_skill_md = $session$Two things sit side by side today, and they don't collapse into each other.
...
-- ---------------------------------------------------------------------------
update public.training_sessions_catalog
   set mental_skill_md = $session$In the car on the way to the rink. The room before warmups. Your internal monologue is running, and you haven't noticed what it sounds like.

Listen. If it sounds like a job interview — proving you belong, justifying your spot, earning the next shift — you're auditioning. Most athletes never hear it. They just feel tight and don't know why.
