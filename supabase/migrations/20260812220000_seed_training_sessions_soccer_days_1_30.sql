-- =============================================================================
-- Migration: 20260812220000_seed_training_sessions_soccer_days_1_30.sql  (FV-77)
--
-- Purpose (soccer go-live, KC founder sign-off 2026-08-12):
--   Seed soccer's 30 daily training rows into public.training_sessions_catalog
--   so the catalog has (day_number, sport='soccer') coverage for days 1-30,
...
--   PR-05/PR-07), not because it carries anything soccer-specific.
--
--   A future FV-430-family content UPDATE with no sport filter (the
--   established pattern) will continue to cover these newly-seeded soccer
--   rows automatically — no separate soccer content-maintenance path is
...
--
-- Safety / risk:
--   Additive only — a pure INSERT of net-new (day_number, sport='soccer')
--   rows. Cannot orphan or alter any existing row (no UPDATE/DELETE).
--   Grepped supabase/migrations/**/*.sql for "soccer" before writing this:
--   zero prior hits, confirming soccer currently has no
--   training_sessions_catalog rows at all — this migration is the first.
--
-- Order (FV-77/FV-81 go-live PR chain):
--   Merges after FV-72..76 (soccer content authoring) and the soccer render,
--   and before/with the SUPPORTED_SPORTS flip + this file's companion
--   migration, 20260812230000_soccer_db_enablement.sql (which widens
...
--   sport is unconstrained `text` on this table (no CHECK), so soccer
...
select
  day_number, 'soccer', title, mental_skill_md, scripture_ref, scripture_text, journal_prompt
