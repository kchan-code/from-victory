...
## Beta backend — 2026-09-22 (KC: cloudflared authorized, Paid Apps Agreement Active)
- Named tunnel **fv-beta** (`6a54e8f8-5a91-461f-b863-a2b5c254fbf4`) created; `~/.cloudflared/config.yml` ingress `beta.fromvictoryapp.com → http://localhost:3590` (`httpHostHeader` set); `cloudflared tunnel run fv-beta` running (4 edge connections registered). Credentials JSON lives in `~/.cloudflared`, never printed.
...
## TESTFLIGHT AVAILABLE — 2026-09-22 14:02 PT
- Build **1.0 (4)** (ASC build id `4cab8497-c05c-4c04-8dcd-76c4c5935471`) processed: `processingState=VALID`, attached to internal group **"First testers"** (hasAccessToAllBuilds) → visible in KC's and Parker's TestFlight app. Not submitted for external testing or App Review.
- Source of record: native `chore/fv-594-testflight-beta-build` @ `4c45345` (#533 tip `8d8d1f8` + build bump), web RC `d2f76d6` served from `beta.fromvictoryapp.com` (local RC server + local Supabase behind tunnel `fv-beta`). Reviews: RC #534 qa + privacy APPROVED @ d2f76d6; #537 qa-reviewed; #538 qa + privacy APPROVED.
- Phone test prerequisites still on KC: finish `subs` / `prices --apply` / `offers --apply`; sandbox tester Apple ID signed in on the phone; Sandbox notification URL set. Keep the tunnel (`cloudflared tunnel run fv-beta`) and `rc-web-local` running for the test window.
- 14:10 PT — `subs` complete (KC-run): all 10 products exist with en-US localizations. Verified read-only: periods ONE_MONTH/ONE_YEAR as designed, `familySharable=false` on all ten, levels cap5→1 … cap1→5, state MISSING_METADATA (price, availability, review screenshot pending). Ids: 1.m 6814844760 · 1.y 6814846005 · 2.m 6814846226 · 2.y 6814846092 · 3.m 6814846261 · 3.y 6814846263 · 4.m 6814846159 · 4.y 6814846119 · 5.m 6814846322 · 5.y 6814846324.
- Price points (read-only, USA, 800 points): an exact **$5.00** point exists (proceeds **$4.25 = 85%** → the Small Business Program 15% rate is in effect on this account; resolves Open Item P4 as "enrolled/applied", verify in ASC Agreements if needed). The script picks the highest point ≤ web price, so monthly tiers land at $5/$8/$11/$14/$17 where such points exist (the proposal's $4.99 was a conservative floor). First `prices --apply` failed with Apple's generic "error occurred while processing the pricing information" — the subscription had no availability record; script reordered to create USA availability first, then the price (fallback: omit the territory relationship).
- Review screenshot: `~/.private_keys/fv-review-subscribe.png` (1290×2796) captured from the local RC `/subscribe` under the iOS-shell UA with the StoreKit bridge stubbed for rendering only (a plain browser shows "Subscriptions aren't available in this version yet." because `window.Capacitor` is absent — expected, not a defect; the real shell has the bridge). Uploaded via `screens … --apply`.
