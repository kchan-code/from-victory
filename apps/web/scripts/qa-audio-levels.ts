...
// measureLoudness (ffmpeg ebur128 stderr parsing) lives in scripts/lib/ebur128.ts
// so bakeoff-voices.ts (FV-285) can reuse it without duplicating the parser.
...
async function measureSlug(
  ...
  const { lufsI, truePeak, error } = await measureLoudness(filePath);
