// Soccer pregame compositional clips (FV-78/79, v2 DORMANT) — 45 hard-moment
// cells (40 selectable + 5 clinically withheld), wired from the KC-ratified
// docs/soccer-module-map.md taxonomy. The soccer analog of
// clips-lacrosse.ts. Kept in a sibling file to stay out of the clips.ts hot
// file. Registered into CLIP_SCRIPTS via `...SOCCER_PREGAME_CLIP_SCRIPTS` in
// clips.ts. The 32 VIZ clips (4 position flagships + 28 positive-play
// library entries) live in clips-viz-soccer.ts, registered separately via
// `...SOCCER_VIZ_CLIP_SCRIPTS`.
//
// PRE-BOOK STRUCTURAL SEEDS: docs/scripts/soccer.md is the SOURCE OF TRUTH
// for this prose. The generator reads book prose (and structure — FV-302) at
// render time via loadBookProse, so these strings are the scaffold the book
// overrides, seeded from the de-corned 6-line HM shape
...
// Only the 5 clinically gated shootout/yips cells carry a 7th worth-truth
// line. Slug scheme: hm-soc-{position}-{frag} (soccer owns soc- / hm-soc- /
// viz-soc- — soccer-module-map.md). Positions: forward (fwd) / midfielder
// (mid) / defender (def) / goalkeeper (gk). Audio render is DEFERRED (the
// sport is DORMANT) — this file is the TTS INPUT, no MP3s yet.
//
// CLINICAL GATES (docs/soccer-module-map.md — the FV-119 pattern):
//   ⚠⚠ WITHHELD (shootout + handling-yips — authored here, absent from the
//   Step-02 picker until clinical sign-off):
//     hm-soc-fwd-shootout, hm-soc-mid-shootout, hm-soc-def-shootout,
//     hm-soc-gk-shootout, hm-soc-gk-handling-yips
//   Never name "the yips". Intrusive-thought lines render only as thoughts
//   to reject ("The thought hits: ..."), never as labels.
//
// SCOPE: soccer only (per docs/soccer-module-map.md).
...
// All 45 cells share the de-corned 6-line structure (7 lines for the 5
// clinically gated cells). The factory keeps the engineering metadata
// (marks, instructions, pauses) uniform so the book's in-sync override maps
// 1:1 per line.
...
type SoccerHmSeed = {
  ...
  next: string;
  /** Optional 7th worth-truth line — GATED cells ONLY. */
  worth?: string;
};
...
// ── Forward — 10 cells — FV-78/79 ────────────────────────────────────────

const FORWARD_HM_SEEDS: SoccerHmSeed[] = [
  {
...
// ── Midfielder — 10 cells — FV-78/79 ─────────────────────────────────────

const MIDFIELDER_HM_SEEDS: SoccerHmSeed[] = [
  {
...
// ── Defender — 10 cells — FV-78/79 ───────────────────────────────────────

const DEFENDER_HM_SEEDS: SoccerHmSeed[] = [
  {
...
// ── Goalkeeper — 10 cells — FV-78/79 ─────────────────────────────────────

const GOALKEEPER_HM_SEEDS: SoccerHmSeed[] = [
  {
...
// ── ⚠⚠ WITHHELD clinically-gated cells (5, FV-119 pattern) ─────────────────
// Authored so the grid + integrity test are complete; UNREACHABLE from the
// Step-02 picker — omitted from every roleAdversities array until KC +
// clinical sign-off. Carries the authorized worth register (7th line).
// See docs/scripts/soccer.md "Hard Moment Clips — Clinically Withheld" and
// the "Clinical routing memo" beneath it.
...
// 45 hard-moment cells (40 selectable + 5 withheld). The 32 VIZ clips (4
